...
class AbstractPropertiesData(AbstractProperties):
    '''
...
#        self._properties = {}
        ...
        super(AbstractPropertiesData, self).__init__(properties=properties,
                                                     source=source, copy=copy)
        
        if source is not None:
            if not isinstance(source, AbstractPropertiesData):
                raise ValueError(
"ERROR: source must be a subclass of 'AbstractPropertiesData'. Got {!r}".format(
...
#    #--- End: def
    ...
    def set_data(self, data=None, copy=True):
