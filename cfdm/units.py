...
If the *copy* parameter is False, then a numpy array is modified in
place, without any copying overheads:

>>> Units.conform(a,
                  Units('days since 2000-12-1'),
                  Units('days since 2001-1-1'), copy=False)
...
converted to floats (see the *copy* parameter).

:Examples 1:

>>> b = Units.conform(a, Units('kilometres'), Units('metres'))
...
:Parameters:
    ...
    copy: `bool`, optional
        If False and `x` is a numpy array then change it in place,
...
'''
        ...
        for attr in self.__dict__:
            try:
                string.append("{}.{} = {!r}".format(prefix, attr,
                                                    getattr(self, attr)))
            except AttributeError:
                pass
        #--- End: for
