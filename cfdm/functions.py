...
def LOG_SEVERITY_LEVEL(*log_level):
    ...
    For example, when set to 'WARNING' (or 2), all 'DEBUG' and 'INFO'
    messages will be supressed, but all others will emerge, & when set
    to 'DEBUG' (or 0), all log messages are displayed. Note that it is
    also possible to disable all log messages, even those marked as
    'CRITICAL', via specifying 'DISABLE' (or 5), an effective highest level.
    ...
    The default level set by cf-python is 'WARNING' (2).
    ...
    :Parameters:
        ...
        log_level: `str` ot `int`, optional
            ...
            'ERROR', or 'CRITICAL', or an integer from 0 to 4 corresponding
            to each of these, where 0 is the lowest severity ('DEBUG') and 5
            the highest ('CRITICAL'). You may also specify 'DISABLE' (5) to
            disable all logging messages regardless of their level.
    ...
    **Examples:**
    ...
    >>> LOG_SEVERITY_LEVEL()
    'ERROR'
    >>> LOG_SEVERITY_LEVEL(0)  # set to 'DEBUG' via corresponding integer
    'ERROR'
    >>> LOG_SEVERITY_LEVEL()
    'DEBUG'
    '''
    ...
    if log_level:
        ...
        if level in valid_log_levels:
            CONSTANTS['LOG_SEVERITY_LEVEL'] = level
            _reset_log_severity_level(level)
        else:
            print(
                "Logging level '{}' not one of the valid values '{}', or a "
                "corresponding integer of 0 to {} respectively (increasing "
                "with severity). Value remains as it was, at:".format(
...
def _reset_log_severity_level(level, logger=None):
    ...
    equivalent integer (see valid_log_levels in LOG_SEVERITY_LEVEL
