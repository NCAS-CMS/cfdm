...
<CF Field: pmsl(30, 24)>
        ...
        # ------------------------------------------------------------        
        nc = self._NetCDFArray.file_open(filename, 'r')
        g['nc'] = nc
...
#        f.Units = f_Units
        ...
        for ncdim in field_ncdimensions:
            if ncdim in nc.variables:
                ...
                domain_axis = self._DomainAxis(coord.size)
...
#            bounds.Units = b_Units
            ...
            if bounds.ndim > c.ndim:
                c_ncdims = nc.variables[ncvar].dimensions
                b_ncdims = nc.variables[ncbounds].dimensions
                if c_ncdims != b_ncdims[:c.ndim]:
                    axes = [c_ncdims.index(ncdim) for ncdim in b_ncdims[:c.ndim]
                            if ncdim in c_ncdims]
                    if len(axes) == c.ndim:
                        axes.extend(range(bounds.ndim - c.ndim, 0))
...
:Examples: 
    ...
    '''
        ...
        filearray = self._NetCDFArray(file=g['filename'],
                                      ncvar=ncvar,
                                      dtype=dtype,
                                      ndim=ndim,
                                      shape=shape,
    ...
    def _read_create_Data(self, array=None, units=None, calendar=None,
                          fill_value=None):
        '''
        '''
