...
def _make_gathered_file(filename):
    ...
    aux9[...] = numpy.arange(
        time.size * height.size).reshape(time.size, height.size)
...
def _make_geometry_1_file(filename):
    ...
    n.Conventions = 'CF-'+VN
    n.featureType = 'timeSeries'
    n.comment = ("Make a netCDF file with 2 node coordinates variables, "
...
def _make_geometry_2_file(filename):
    ...
    n.Conventions = 'CF-'+VN
    n.featureType = 'timeSeries'
    n.comment = ('A netCDF file with 3 node coordinates variables, only '
...
def _make_geometry_3_file(filename):
    ...
    n.Conventions = 'CF-'+VN
    n.featureType = 'timeSeries'
    n.comment = ("A netCDF file with 3 node coordinates variables, each of "
...
def _make_geometry_4_file(filename):
    ...
    n.Conventions = 'CF-'+VN
    n.featureType = 'timeSeries'
    n.comment = ("A netCDF file with 2 node coordinates variables, none of "
...
# def _make_grouped_1_file(filename):
#     nc = netCDF4.Dataset(filename, "w")
#
#     # ----------------------------------------------------------------
#     # Root Group
#     # ----------------------------------------------------------------
#     lat = nc.createDimension('lat', 5)
...
# grouped_1_file = _make_grouped_1_file('grouped_1.nc')
