...
<CF Field: pmsl(30, 24)>
    ...
    "WARNING: Moving unsupported units to 'nonCF_Units': {0}".format(
        ...
        for ncvar in variables.copy():
            ...
            if compressed_ncdims is not None:
                ...
                _parse_compression_gathered(nc, ncvar, attributes, compressed_ncdims)
    ...
    "WARNING: Missing bounds variable {!r} in {}".format(bounds, filename))
                    ...
                    if 'formula_terms' in attributes[ncvar]:
                        ...
                        self._formula_terms_variables(nc, ncvar, formula_terms,
                        ...
                        if bounds is not None and 'formula_terms' in attributes[bounds]:
                            ...
                            self._formula_terms_variables(nc, ncvar,
    ...
    def _parse_compression_gathered(self, nc, ncvar, attributes,
                                    compressed_ncdims):
        '''
        '''
        g = self.read_vars
    ...
    def _parse_DSG_contiguous_compression(self, nc, ncvar, attributes,
                                          sample_ncdimension):
        '''
        ncvar: `str`
    ...
    def _parse_DSG_indexed_compression(self, nc, ncvar, attributes,
                                       instance_ncdimension):
        '''
        ncvar: `str`
    ...
    def _parse_DSG_indexed_contiguous_compression(self, nc, ncvar,
                                                  sample_ncdimension,
                                                  instance_ncdimension):
        ''':Parameters:
    ...
    def _formula_terms_variables(self, nc, ncvar, formula_terms,
    ...
    def _create_Field(self, nc, data_ncvar, attributes,
                      dimension_coordinates, auxiliary_coordinates,
                      cell_measures, domain_ancillaries,
                      field_ancillaries, coordref_parameters,
    ...
    "WARNING: Moving unsupported cell methods to 'nonCF_cell_methods': {0!r}".format(
        ...
        for ncdim in field_ncdimensions:
            if ncdim in nc.variables:
                ...
                if ncdim in dimension_coordinates:
                    ...
                    coord = self._create_bounded_item(ncdim, attributes, f,
                                                      dimension=True,
                                                      verbose=verbose)
        ...
        if coordinates is not None:
            ...
            for ncvar in re.split('\s+|\s*,\s*', coordinates):
                ...
                aux_ncdimensions = self._ncdimensions(ncvar)
    ...
    #            print 'dimensions=',dimensions
            ...
            for term, ncvar in g['formula_terms'][coord.ncvar]['coord'].iteritems():
                ...
                if not axes:
                    ...
                    if ncvar not in coordref_parameters:
                        variable = _create_Data(ncvar, attributes)
        ...
        if grid_mapping is not None:
            self._create_grid_mapping_ref(f, grid_mapping, attributes, ncvar_to_key)
        ...
        if measures is not None:
            ...
            for measure, ncvar in zip(measures[1::2], 
                ...
                if ncvar in cell_measures:
                    ...
                    cell = self._create_item(ncvar, attributes, f, cell_measure=True)
        ...
        if ancillary_variables is not None:
            ...
            for ncvar in re.split('\s+|\s*,\s*', ancillary_variables):
                ...
                if ncvar in field_ancillaries:
                    ...
                    field_anc = self._create_item(ncvar, attributes, f,
...
:Returns:
    ...
    def _create_item(self, ncvar, attributes, f, cell_measure=False,
                     field_ancillary=False):
        '''
    ...
    def _create_grid_mapping_ref(self, f, grid_mapping, attributes, ncvar_to_key):
        '''
...
[<CF Field: air_pressure(30, 24)>,
 ...
 <CF Field: potential_temperature(19, 30, 24)>]
    ...
    #        _close_netcdf_file(filename)
        ...
        # ---------------------------------------------------------------
        self._create_global_properties(fields)
    ...
    def _write_attributes(self, netcdf_var, netcdf_attrs):
        '''
    ...
    def _grid_ncdimensions(self, f, key, axis_to_ncdim):
...
:Returns:
    ...
    def _variable_ncvar(self, variable, default):
        '''
    ...
    def _write_dimension_coordinate(self, f, axis, coord, key_to_ncvar, axis_to_ncdim):
        '''
    ...
    '''       
        ...
        if create:
            ncdim = self._variable_ncvar(coord, default='coordinate')
            ...
            ncdimensions = self._grid_ncdimensions(f, axis, axis_to_ncdim)
            ...
            self._create_netcdf_variable(ncdim, ncdimensions, coord, 
                                         extra=extra)
        else:
...
dictionary.
    ...
    >>> extra = _write_bounds(c, ('dim2',))
        ...
        if self._seen(bounds, ncdimensions):
            ...
            self._create_netcdf_variable(ncvar, ncdimensions, bounds, omit=omit)
        #--- End: if
    ...
    :Returns:
        ...
        if not self._seen(coord, ()):
            ncvar = self._variable_ncvar(coord, default='scalar')
            ...
            self._create_netcdf_variable(ncvar, (), coord, extra=extra)
    ...
    >>> coordinates = _write_auxiliary_coordinate(f, 'aux2', coordinates)
        ...
        if self._seen(coord, ncdimensions):
            ...
            ncvar = self._variable_ncvar(coord, default='auxiliary')
            ...
            # Create a new auxiliary coordinate variable
            self._create_netcdf_variable(ncvar, ncdimensions, coord, extra=extra)
        #--- End: if
    ...
    >>> _write_domain_ancillary(f, 'cct2', anc)
        ...
        if not create:
            ...
            ncvar = self._variable_ncvar(anc, default='domain_ancillary')
    
            # If this domain ancillary has bounds then create the bounds
            # netCDF variable
            self._write_bounds(anc, ncdimensions, ncvar)
    
            self._create_netcdf_variable(ncvar, ncdimensions, anc, extra={})
        #--- End: if
    ...
    >>> ncvar = _write_field_ancillary(f, 'fav2', anc, key_to_ncvar, axis_to_ncdim)
        ...
        if not create:
            ...
            ncvar = self._variable_ncvar(anc, 'ancillary_data')
            self._create_netcdf_variable(ncvar, ncdimensions, anc)
    ...
    "Can't create a cell measure variable without a 'measure' attribute")
            ...
            ncvar = self._variable_ncvar(msr, 'cell_measure')
    
            self._create_netcdf_variable(ncvar, ncdimensions, msr)
        #--- End: if
    ...
    '''
        ...
        if self._seen(ref):
            ...
            ncvar = self._variable_ncvar(ref, 'grid_mapping')
    ...
    def _create_netcdf_variable(self, ncvar, dimensions, cfvar, omit=(),
                                extra={}, data_variable=False):
...
created. The ``seen`` dictionary is updated for *cfvar*.
    ...
    message+". Unlimited dimension must be the first (leftmost) dimension of the variable. Consider using a netCDF4 format.")
        ...
        self._write_attributes(g['nc'][ncvar], netcdf_attrs)
    ...
    :Parameters:
        ...
        if strlen > 1:
            ...
            array = self._character_array(data.array)
    ...
    #                value = Data.asdata(value)
        ...
        # ----------------------------------------------------------------
        ncvar = self._variable_ncvar(f, 'data')
    ...
    def _create_global_properties(self, fields):
