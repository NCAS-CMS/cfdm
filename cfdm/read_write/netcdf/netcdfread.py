...
class NetCDFRead(IORead):
    ...
    @classmethod
    def dtype(cls, dataset, dataset_type):
    ...
    def read(
        ...
        # valid.
        #
        # Note that the `dtype` method is much faster than the
        # `file_open` method at returning for unrecognised types.
        # ------------------------------------------------------------
        dtype = self.dtype(dataset, dataset_type)
        if not dtype:
