...
The cfdm package was not created with performance

* In-place operations

  Some methods (such as `~Field.squeeze`, `~Field.transpose` and
  `~Field.insert_dimension`) have an option to perform the operation
  in-place, rather than creating a new, independent object.
